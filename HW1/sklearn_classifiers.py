...
classifiers = [
    ...
    SVC(kernel="linear", C=0.9),
    SVC(gamma=2, C=1),
    RandomForestClassifier(max_depth=7, n_estimators=70, max_features=1, criterion='entropy'),
    MLPClassifier(hidden_layer_sizes=[100], activation='relu',
                  alpha=0.003, max_iter=9999, solver='sgd'),
    AdaBoostClassifier(n_estimators=60, learning_rate=0.9),
    GaussianNB(var_smoothing=1e-7),
    QuadraticDiscriminantAnalysis(tol=1.0e-3),
    GaussianProcessClassifier(optimizer='fmin_l_bfgs_b', max_iter_predict=150),
]
names = [
    "Logistic-Regr",
    "KNeighbors-5 ",
